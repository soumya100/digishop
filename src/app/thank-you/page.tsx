...
const ThankYouPage = async ({ searchParams }: PageProps) => {
    ...
    const rawOrders = orders as unknown as Order[]
    const [order] = rawOrders
                    ...
                    <div className='lg:col-start-2'>
                        ...
                        {order?._isPaid ? (
                            <p className='mt-2 text-base text-muted-foreground'>
                                Your order was processed and your assets are available to download below. We&apos;ve sent
                        ...
                        <div className='mt-16 text-sm font-medium'>
                            ...
                            <ul className='mt-6 divide-y divide-gray-200 border-t border-gray-200 text-sm font-medium text-muted-foreground'>
                                {products.map((product) => {
                                    ...
                                    const productFile = product.product_files as ProductFile
                                    const downloadUrl = productFile?.url ?? '#'

                                    const imageEntry = product.images?.[0]
                                    const imageUrl =
                                        typeof imageEntry?.image !== 'string' ? imageEntry?.image?.url : undefined
